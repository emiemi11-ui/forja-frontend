...
import { Dumbbell, TrendingUp, Clock, Flame, ArrowLeft } from 'lucide-react';
import { getCoachAthleteHistory7Days } from '../../../shared/api/index.js';
...
export default function CoachAthleteHistoryPage() {
  ...
  useEffect(() => {
    if (!athleteId) return;
    getCoachAthleteHistory7Days(athleteId)
      .then((r) => setPayload(r.data || { athlete: null, days: [] }))
      .catch((err) => {
        showToast(err.response?.data?.error || 'Eroare la încărcare', '❌');
        setPayload({ athlete: null, days: [] });
      });
  }, [athleteId]);
  ...
  return (
    <AnimatedPage>
      ...
      >
        <ArrowLeft size={14} /> Înapoi la atleți
      </button>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <div style={{ fontFamily: 'var(--fm)', fontSize: 10, letterSpacing: 3, color: 'var(--c-lime-d)', textTransform: 'uppercase', fontWeight: 700, marginBottom: 6 }}>
              Atlet · {athlete?.name || '—'}
            </div>
            <div style={{ fontFamily: 'var(--fd)', fontSize: 28, fontWeight: 900, color: 'var(--hero-text)', letterSpacing: 0.5, marginBottom: 16 }}>ISTORIC ANTRENAMENTE</div>
          </motion.div>
          ...
          {days.every((d) => d.workouts === 0) ? (
            <div style={{ padding: '40px 12px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--c-ink3)', fontFamily: 'var(--fm)' }}>
                {athlete?.name || 'Atletul'} nu a finalizat nicio sesiune în această perioadă.
              </div>
            </div>
          ) : (
            <div>
