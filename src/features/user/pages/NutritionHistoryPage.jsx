...
import { Apple, Flame, Drumstick, Wheat } from 'lucide-react';
import { getNutritionHistory7Days } from '../../../shared/api/index.js';
...
// Culoare pe baza procentajului față de target — verde dacă e între 80-110%, lime dacă e între 50-80%, coral altfel
...
export default function NutritionHistoryPage() {
  ...
  useEffect(() => {
    getNutritionHistory7Days()
      .then((r) => setPayload(r.data || { targets: {}, days: [] }))
      .catch(() => {
        showToast('Eroare la încărcare', '❌');
        setPayload({ targets: {}, days: [] });
      });
  }, []);
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <div style={{ fontFamily: 'var(--fm)', fontSize: 10, letterSpacing: 3, color: 'var(--c-coral)', textTransform: 'uppercase', fontWeight: 700, marginBottom: 6 }}>nutriție</div>
            <div style={{ fontFamily: 'var(--fd)', fontSize: 32, fontWeight: 900, color: 'var(--hero-text)', letterSpacing: 0.5, marginBottom: 16 }}>ISTORIC NUTRIȚIE</div>
          </motion.div>
          ...
          {days.every((d) => d.meals === 0) ? (
            <div style={{ padding: '40px 12px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--c-ink3)', fontFamily: 'var(--fm)', marginBottom: 14 }}>
                Adaugă alimentele consumate pentru a vedea istoricul aici.
              </div>
              <button className="btn btn-lime btn-sm" onClick={() => navigate('/app/nutrition')}>
                🍎 Adaugă aliment
              </button>
            </div>
          ) : (
